...
PNG createSpotlight(PNG image, int centerX, int centerY)
{
  ...
  for (unsigned int  x = 0; x < image.width(); x++) 
  {
	...
	//percentage decrease in Luminance
	if(d <= 160)
	{
		multiplier = 1-(0.005*d); 
	}
	else
	{
		multiplier = 0.2;
...
{
	double hor1=11, hor2=371, hbl=216;//hue of orange and blue respectively  
	for (unsigned int  x = 0; x < image.width(); x++) 
  	{
    		for (unsigned int y = 0; y < image.height(); y++)
    		{
			HSLAPixel & pixel = image.getPixel(x, y);
			(abs(pixel.h - hor1) < abs(pixel.h - hbl)) || ((abs(pixel.h - hor2)) < abs(pixel.h - hbl)) ? pixel.h = hor1 : pixel.h = hbl; 
    		}
  	}
